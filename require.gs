...
// NOTE: function declaration (not const/var) is intentional — GAS V8 hoists function declarations
// globally BEFORE any file's top-level code runs, including the var/const assignments.
// A function declaration is always callable, even before the line it appears on is "reached".
// eslint-disable-next-line no-unused-vars
function debugLog() {} // no-op by default; replaced below after module system initializes
...
function require(moduleName) {
  // ⚠ Guard: non-string moduleName causes name.replace crash in normalize()
  if (typeof moduleName !== 'string') {
    throw new Error(`require() expects a string module name, got ${typeof moduleName}: ${JSON.stringify(moduleName)}`);
  }
...
function __defineModule__(moduleFactory, explicitName, options) {
  // Argument validation — prevents "[object Object]" module names
  if (typeof moduleFactory !== 'function') {
    throw new Error(`__defineModule__: first argument must be a function, got ${typeof moduleFactory}`);
  }
  // ⚠ Handle common misuse: __defineModule__(factory, true/false) or (factory, {options})
  if (typeof explicitName === 'boolean') {
    options = { loadNow: explicitName };
    explicitName = undefined;
  } else if (typeof explicitName === 'object' && explicitName !== null) {
    options = explicitName;
    explicitName = undefined;
  } else if (explicitName != null && typeof explicitName !== 'string') {
    throw new Error(`__defineModule__: explicitName must be a string or undefined, got ${typeof explicitName}`);
  }
...
(function() {
   ...
   * First-responder-wins: first handler to return a truthy response wins;
   * remaining handlers are not called. Handlers return null to yield.
   */
  globalThis.__doGet_dispatcher = function(e) {
    ...
    debugLog(`[DISPATCH] Dispatching doGet to ${handlers.length} handler(s) (first responder wins)`);
    ...
    for (const handlerInfo of handlers) {
      try {
        ...
        if (response) {
          debugLog(`  ✓ ${handlerInfo.module} claimed doGet`);
          return response;
        }
      } catch (error) {
        errorCount++;
    ...
    debugLog(`[WARN] All doGet handlers returned null or threw (${errorCount} error(s))`);
    return ContentService.createTextOutput(
      JSON.stringify({
        error: true,
        message: 'No doGet handler claimed the request',
  ...
  globalThis.__doPost_dispatcher = function(e) {
    ...
    debugLog(`[DISPATCH] Dispatching doPost to ${handlers.length} handler(s) (first responder wins)`);
    ...
    for (const handlerInfo of handlers) {
      try {
        ...
        const response = handlerInfo.handler(e);
        if (response) {
          debugLog(`  ✓ ${handlerInfo.module} claimed doPost`);
          return response;
        }
      } catch (error) {
        errorCount++;
    ...
    debugLog(`[WARN] All doPost handlers returned null or threw (${errorCount} error(s))`);
    return ContentService.createTextOutput(
      JSON.stringify({
        error: true,
        message: 'No doPost handler claimed the request',
        totalHandlers: handlers.length,
        failedCount: errorCount
      })
    ).setMimeType(ContentService.MimeType.JSON);

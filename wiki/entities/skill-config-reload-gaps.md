...
- **From Session f20f3ade:** Gap #2 (`route`/`backend` no `--reload`) further closed: SKILL.md `route` and `backend` subcommands now both support `--reload` flag with success message "Route/backend set + proxy reloaded" (matching `mode`/`remap` pattern). Backend documentation clarified: `backend` does a **full replace** of the backend URL (not merge/append), which was a documentation bug caught by review-fix. Ollama drift probe added as Step 4 of `diag` — checks whether locally-installed Ollama models match the config's expected models for the active hw tier, printing `✓ model:tag` or `✗ model:tag (installed: other:tag / not found)`.

→ See also: [[sighup-config-reload]], [[config-swap-invariant]], [[declared-rewrites]], [[capability-profile-model-layers]], [[runtime-control]], [[model-map-edit-key-whitelist]], [[model-map-resolve-module]]
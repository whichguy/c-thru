...
- **From Session 408250c4:** Decision resolved and codified in test: the reactive-only branch was chosen. `llm_mode` 4-value enum with `modes[]` sub-maps provides mode-specific overrides at the profile level, while per-request cascade handles failures. No proactive liveness prober. The `test/llm-mode-resolution-matrix.test.js` test suite validates the full resolution semantics. See [[llm-mode-resolution]] for resolver detail.

→ See also: [[capability-profile-model-layers]], [[fallback-event-system]], [[config-swap-invariant]], [[llm-mode-resolution]]
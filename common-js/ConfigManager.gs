  ...
  class ConfigManager {
    ...
    constructor(moduleName, options = {}) {
      ...
      try {
        ...
        this.docProps = null; // Standalone script - no document properties
      }
    ...
    _ensureCache() {
      if (!this._cache || Date.now() - this._cacheTime > this.CACHE_TTL) {
        ...
        this._cache = {
          userDoc: this.docId ? this._filterByPrefix(userProps, `${this.docId}_${this.module}_`) : {},
    ...
    _executePurge(key, scopePriority) {
      ...
      // Skip scopes that are unavailable in standalone scripts
      for (let i = 0; i < scopePriority; i++) {
        const scope = scopes[i];
        if (scope === 'userDoc' && !this.docId) continue;
        if (scope === 'document' && !this.docProps) continue;
     ...
     */
    _getLayerInfo(scope) {
      if (scope === 'userDoc' && !this.docId) {
        throw new Error('userDoc scope requires a container-bound script with an active spreadsheet');
      }
      if (scope === 'document' && !this.docProps) {
        throw new Error('document scope requires a container-bound script (no DocumentProperties in standalone)');
      }
      const layers = {
        userDoc: { 

...
- `.claude/settings.json` (project-level) — persistent project hooks: `SessionStart` → `c-thru-session-start.sh` (10s), `PostCompact` → `c-thru-postcompact-context.sh` (5s). These are version-controlled and not touched by `install.sh`.

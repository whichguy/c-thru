...
- **advisor_panels** — multi-agent panel seats for the **advisors** skill (`/advisors`), keyed by panel role (default `default`) then connectivity mode. Each mode entry has `seats` (2–5 agent names or capabilities) and optional `description`. Resolve without dispatch: `c-thru explain --panel default --mode <m> --tier <t>`. Missing non-default roles fall back to `default` **for the same mode only** (never a different mode's seats). See `skills/advisors/SKILL.md` and `tools/model-map-resolve.js` → `resolveAdvisorPanel`.

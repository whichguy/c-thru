...
**Gemma4:26b (MoE only) returns a completely empty response** — no content, `done_reason: "stop"` after ~49 tokens — when the system prompt is long. The reporter in issue #15428 estimates the threshold at roughly 500 characters (prompts under ~200 chars work; a 2000-char prompt fails). **⚠️ Disputed:** one GGUF vendor (rparo20) was unable to reproduce across five configurations including 1.8–10 KB system prompts, suggesting this may be GGUF-variant or quant-source specific. The issue is labeled "needs more info" with no confirmed root cause from Ollama maintainers. Dense models (31b, e4b) handle the same prompt correctly — the MoE-only nature is confirmed. ([issue #15428](https://github.com/ollama/ollama/issues/15428))
...
| Tool-call parser errors | `e4b` (confirmed, issue #15315); `26b`/`31b` fixed in PR #15254 | High for e4b | Use `26b` or `31b`; avoid e4b for tool dispatch |
...
**Flash Attention: 31b Dense affected, 26b MoE NOT affected.** The hang from hybrid attention architecture affects the Dense 31b. The 26b MoE's expert routing sidesteps the problematic attention layer interaction — FA is safe to enable for `gemma4:26b-a4b`. Keep `OLLAMA_FLASH_ATTENTION=0` only for 31b. ([issue #15368](https://github.com/ollama/ollama/issues/15368))

**Tool calling in Ollama 0.20.x is deeply broken for Gemma4.** Three failure modes: (1) parser crashes with `"gemma4 tool call parsing failed: invalid character"`; (2) streaming tool calls appear in `reasoning` field; (3) `<unused25>` token garbage in tool responses. No Modelfile workaround exists — the fix requires llama.cpp PRs #21326 and #21343. Use llama.cpp-server directly with `--jinja` and these PRs for production tool-calling with Gemma4. ([daniel-farina gist](https://gist.github.com/daniel-farina/87dc1c394b94e45bb700d27e9ea03193))

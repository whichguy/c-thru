...
| `XAI_API_KEY` | API key for xAI Grok (`endpoints.xai`, models `grok` / `grok-4.5`). Sent as `Authorization: Bearer`. Never forwards the client's Anthropic credentials to `api.x.ai`. Required for the `grok` brand agent. Live tool/stream fidelity: set `C_THRU_LIVE_XAI=1` after install to smoke-test (Anthropic Messages on xAI is a legacy compatibility surface). |

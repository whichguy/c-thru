...
| `C_THRU_STATUSLINE_STYLE` | `minimal` \| `default` \| `stats` — statusline content. Overrides `~/.claude/c-thru-statusline.json`. `stats` uses `GET /c-thru/statusline`: **last hop / fallback are session-scoped** when the hook base URL includes `/s/<id>`; **Σ / `usage_window` is the machine-wide lifetime ledger since last clear** (not per Claude session). |
...
| `C_THRU_STATS_RESET` | `never` (default) \| `launch` — when `launch`, POST `/c-thru/stats/clear` once after this process tree's proxy is ready. **Clears the shared `usage-stats.json` lifetime ledger** used by all sessions (and multi-user machines sharing that file) — forensics totals are wiped, not just one chat. Prefer explicit `c-thru stats clear` unless you intentionally want a clean ledger every launch. |
| `CLAUDE_PROXY_USAGE_STATS_FILE` | Override path for the cumulative usage ledger (default `~/.claude/usage-stats.json`). Shared across proxy instances and sessions; clear with `c-thru stats clear` / `POST /c-thru/stats/clear`. |

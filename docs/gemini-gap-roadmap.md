...
| **G14** ✓ | Gemini and Anthropic both count thinking inside the total output ceiling. Gemini 3 Pro cannot fully disable thinking, and supported levels vary by model. | Hermetic level/cap tests plus live `thoughtsTokenCount` probes | **Shipped.** Preserve `max_tokens` as `maxOutputTokens` without inflation; sum `candidatesTokenCount + thoughtsTokenCount` into `output_tokens`; map disable to the nearest supported off/low control and report inexact semantics; honor `display:"omitted"`; expose auto-enable, selected level, thinking-token, and translation-gap telemetry. |

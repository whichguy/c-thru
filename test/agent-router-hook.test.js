...
//   2. prepends a shared-agent-token-authenticated sentinel to the task prompt.
//
...
function runHook(payload, extraEnv = {}) {
  // Override ambient launcher state so marker bytes are deterministic.
  const env = Object.assign({}, process.env, {
    CLAUDE_MODEL_MAP_PATH: MODEL_MAP,
    C_THRU_AGENT_SENTINEL_SECRET: SENTINEL_SECRET,
    C_THRU_AGENT_SENTINEL_SECRET_FILE: path.join(os.tmpdir(), 'c-thru-test-no-such-agent-token'),
  }, extraEnv);
...
function signedMarker(agent) {
  return formatAgentSentinel(agent, SENTINEL_SECRET);
}
...
// ── 1. Every mapped agent → updatedInput.model is a valid Agent-tool enum ─────────
// Claude Code only accepts sonnet|opus|haiku|fable. Brand leaves that *are* those
// ids inject themselves; all other mapped agents use C_THRU_AGENT_FALLBACK_ALIAS.
console.log(`1. Agent subagent_type → valid alias (full fleet, unblocks delegation)`);
const NATIVE_AGENT_ENUM = new Set(['sonnet', 'opus', 'haiku', 'fable']);
...
for (const agent of agents) {
  if (hasCapability(agent)) {
    const expected = NATIVE_AGENT_ENUM.has(agent) ? agent : DEFAULT_ALIAS;
    check(`Agent(${agent}) → model=${expected} (valid, not blocked)`, expected, hookModel(agentPayload(agent)));
  } else {
    check(`Agent(${agent}) → passthrough (no capability mapping)`, '', hookModel(agentPayload(agent)));
  }
}
check('fleet roster is >= 28 agents', 'true', String(agents.length >= 28));
...
console.log('');
console.log('4b. advisor:<model-id> runtime pin → signed sentinel injected without config entry');
{
  ...
  try {
    ...
    const ok = prompt.startsWith(signedMarker('advisor:deepseek-v4-pro:cloud')) ? 'yes' : 'no';
    check('advisor runtime pin prompt carries its signed sentinel', 'yes', ok);
  } finally {
...
console.log('');
console.log('5. updatedInput injects model + signed sentinel prompt, preserves fields');
{
  const out = hookRaw(agentPayload('planner'));
  const parsed = parseJsonOrNull(out) || {};
  const updated = parsed.hookSpecificOutput?.updatedInput || {};
  const decision = parsed.hookSpecificOutput?.permissionDecision || '';
  check('prompt starts with planner signed sentinel', 'yes',
    String(updated.prompt || '').startsWith(signedMarker('planner')) ? 'yes' : 'no');
...
console.log('');
console.log('6. node fallback (jq removed from PATH) emits alias + signed sentinel prompt');
{
  ...
  check('node fallback prompt carries coder signed sentinel', 'yes',
    String(updated.prompt || '').startsWith(signedMarker('coder')) ? 'yes' : 'no');
}

// ── 7. Missing agent token fails closed without losing fallback alias ─────────
console.log('');
console.log('7. missing agent token → no unsigned marker, fallback alias preserved');
{
  const out = hookRaw(agentPayload('coder'), { C_THRU_AGENT_SENTINEL_SECRET: '' });
  const parsed = parseJsonOrNull(out) || {};
  const updated = parsed.hookSpecificOutput?.updatedInput || {};
  check('missing secret still injects enum-safe alias', DEFAULT_ALIAS, updated.model || '');
  check('missing secret adds no forgeable sentinel', 'no',
    String(updated.prompt || '').includes('[[c-thru-agent:') ? 'yes' : 'no');
  check('missing secret preserves original task text', 'yes',
    String(updated.prompt || '').includes('p') ? 'yes' : 'no');
}

// ── 8. Stable token-file path signs without exporting the value ─────────────
console.log('');
console.log('8. stable token file → signed marker');
{
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'c-thru-hook-token-'));
  const tokenPath = path.join(dir, 'proxy.agent-token');
  try {
    fs.writeFileSync(tokenPath, SENTINEL_SECRET, { mode: 0o600 });
    const prompt = hookPrompt(agentPayload('coder'), {
      C_THRU_AGENT_SENTINEL_SECRET: '',
      C_THRU_AGENT_SENTINEL_SECRET_FILE: tokenPath,
    });
    check('token-file hook emits expected signed marker', 'yes',
      prompt.startsWith(signedMarker('coder')) ? 'yes' : 'no');
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
}

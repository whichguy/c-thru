...
function runHook(payload, extraEnv = {}) {
  // Force a guaranteed-nonexistent HMAC key path so the suite is hermetic against
  // ambient session state: when run from inside a live Claude Code session,
  // CLAUDE_CONFIG_DIR (inherited via ...process.env) points at that session's own
  // ephemeral dir, which may already contain a real agent-hmac.key generated by other
  // session activity — the hook would then stamp a real HMAC tag onto the sentinel and
  // break the exact-string assertions below, none of which test the tagged path.
  const env = Object.assign({}, process.env, {
    CLAUDE_MODEL_MAP_PATH: MODEL_MAP,
    CLAUDE_PROXY_AGENT_HMAC_FILE: path.join(os.tmpdir(), 'c-thru-test-no-such-hmac-key'),
  }, extraEnv);

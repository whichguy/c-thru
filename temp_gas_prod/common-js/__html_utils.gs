function _main(
  ...
  function include(filename) {
    // Delegate to includeNested for recursive include support
    return includeNested(filename);
  }
   ...
   * 
   * ������ WARNING: Use with caution. Circular includes will throw an error.
  ...
  function includeNested(filename, depth) {
    ...
    // Use getRawContent() to get file content without HTML validation
    // This allows including raw JS/CSS files that don't have proper HTML structure
    let content = HtmlService.createTemplateFromFile(filename).getRawContent();
    
    const includePattern = /<\?!=\s*(?:include|includeNested)\(['"]([^'"]+)['"]\)\s*\?>/g;
